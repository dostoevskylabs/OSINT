...
sub child
{
		...
		my ($sock, $resp, $proxy);
			...
			($resp, $err) = &sendsmtp($sock, "MAIL FROM: <$from>");
			unless ($resp)
			{
				close $sock;
			}
			...
			($resp, $err) = &sendsmtp($sock, "RCPT TO: <$email>");
			unless ($resp)
			{
				close $sock;
			}
			...
			my $message = &message($helo, $name, $addr, $fname, $from, $info);
#print $message;
			$message .= $CRLF . '.';
...
sub readsmtp
{
	...
	if ($s->can_read($c{'timeout'})) {
		...
		unless ($line && $line =~ m!^\d\d\d!) {
			...
			return (undef, "Сервер вернул нераспознанный ответ"); }
